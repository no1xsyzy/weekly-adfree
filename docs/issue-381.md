...
开源的 Nginx 访问日志分析与可视化面板，提供实时统计、PV 过滤、IP 归属地、客户端解析。 （[@likaia](https://github.com/ruanyf/weekly/issues/8696) 投稿）
...
这个网站可以查看 BTC 区块链的 OP\_RETURN 字段，该字段记录了一段文本，只要发上区块链就永远不会删除和修改。（[@blueslmj](https://www.evermsg.com/zh) 投稿）
